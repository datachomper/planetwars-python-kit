...
# to run MyBot against a specified bot on all maps, pass bot name as a parameter, for instance ./batch.sh Dual
...
for file in example_bots/${BOT}Bot.jar
do
   player_1_counter=0
   ...
   for i in {1..100}
   do
      RES=`java -jar tools/PlayGame.jar maps/map$i.txt 1000 200 games.log "java -jar $file" "python MyBot.py" 2>&1 | tail -n 3 | grep "Turn\|Player"`
